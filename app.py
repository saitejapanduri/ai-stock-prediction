...
# FIX: drop today's row if the IST market hasn't closed yet, so indicators
# and predictions are anchored to the last FULLY completed trading day.
# Uses explicit IST timezone so this works correctly regardless of what
# timezone the hosting server (e.g. Streamlit Cloud, which runs UTC) is in.
from zoneinfo import ZoneInfo

ist_now = datetime.datetime.now(ZoneInfo("Asia/Kolkata"))
today_ist = ist_now.date()
market_close_ist = datetime.time(15, 30)  # NSE closes 3:30 PM IST

if data.index[-1].date() == today_ist and ist_now.time() < market_close_ist:
    data = data.iloc[:-1]
